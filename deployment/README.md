...
Consumers) have an interest in automatically discovering DSP-endpoints and inferring knowledge about their expected behavior.
...
Endpoint deployment is a detail that is left to the discretion of each participant. The DSP-spec accounts for this with
...
difference that two entries are returned with two separate `<base>` URLs - one for each protocol version.
